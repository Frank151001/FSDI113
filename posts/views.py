...
class ArchivedPostListView(LoginRequiredMixin,ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        archivied_status = Status.objects.get(name="archivied")
        context["post_list"]=Post.objects.filter(status=archivied_status).filter(author=self.request.user).order_by("created_on").reverse
        return context
